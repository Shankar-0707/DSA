...
public:
    ...
    int n;
    vector<Node> segTree; // iska size 4*n hoga 
    ...
    vector<int> longestRepeating(string s, string queryCharacters, vector<int>& queryIndices) {
        // Brute Force Method 
        // For Q queries check everytime the whole array for longest susbtring that will take O(Q*n) complexity 

        n = s.size();
        segTree.assign(4*n, Node()); // abhi ke liye sbhi me dummy Nodes daldi 
        ...
        for(int i=0; i<k; i++){
            ...
            result[i] = segTree[0].maxLen; // root always covers complete string info
        }
